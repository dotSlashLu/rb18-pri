...
#[inline]
pub fn csum_fold_32(mut csum: u32) -> u32 {
    for _i in 0..4 {
        if csum >> 16 != 0 {
            csum = (csum & 0xffff) + (csum >> 16);
        }
...
pub fn csum_ipv4_l4(data_start: *mut u32, data_size: usize, csum: *mut u64, iph: *mut iphdr) {
    ...
    unsafe {
        ...
        *csum = bpf_csum_diff(&mut 0, 0, &mut tmp, size_u32, *csum as u32) as u64;
        ...
        //   tmp = __builtin_bswap32((__u32)(data_size));
        tmp = (data_size as u32).swap_bytes();
...
#[inline]
pub fn csum_ipv4_l4_6(csum: u32, o: u32, n: u32) -> u32 {
    // accepts 
    //  csum(le)
    //  o(le)
    //  n(be)
    // returns
    //  csum(le)
    _ = printk!("csum %x o %x n %x", csum, o, n);
    let mut sum = o + (!n.swap_bytes() & 0xffff);
    _ = printk!("0. %x, %x", !n.swap_bytes(), !n.swap_bytes() & 0xffff);
    _ = printk!("1. %x", sum);
    sum += csum;
    _ = printk!("2. %x", sum);
    sum = (sum & 0xffff) + (sum >> 16);
    _ = printk!("3. %x", sum);
    // let res = sum + (sum >> 16) - 1;
    // printk!("4. %x", res);
    sum
}